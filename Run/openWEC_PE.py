...
class Ui_MainWindow(QtGui.QMainWindow):
    ...
    def makeMesh(self,MainWindow):
        ...
        ne.createMeshOpt(zG,nPanels/2,0,float(self.rhoBox.text()),nbody=2,xG=[-(length+spacing)/2.0,(length+spacing)/2.0])        
    ...
    def runNemohCode(self):
        ...
        (self.Ma,self.Bhyd,omeg) = pn.getAB(dof,nbody=2,sel=self.dof.index(1))
        self.freq = omeg/(2*np.pi)
        (self.Fe,self.Fpha) = pn.getFe(dof,nbody=2,sel=self.dof.index(1))
